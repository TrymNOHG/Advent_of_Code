...
def part2(file):
    ...
    down_move = [(starting_pos[0], starting_pos[1] + 1), 0]
    right_move = [(starting_pos[0] + 1, starting_pos[1]), 90]
    up_move = [(starting_pos[0], starting_pos[1] - 1), 180]
    ...
    while next_pipe != 'S':
        ...
        # if next_pipe == 'S':
        #     _, start_pipe_coords, new_rotation = is_move_valid(current, next_move, '|')
        #     current = next_move
        #     next_move = start_pipe_coords
        #     next_pipe = grid[next_move[1]][next_move[0]]
        #     rotation = (rotation + new_rotation) % 360
...
def part3(file):
    ...
    valid_start_dir = [valid_start_dir[0]]
    for start_pipe_coords in valid_start_dir:
        current = starting_pos
        next_move = start_pipe_coords
        next_pipe = grid[start_pipe_coords[1]][start_pipe_coords[0]]
        ...
        while next_pipe != 'S':
            ...
            if next_pipe == 'S':
                _, start_pipe_coords, _ = is_move_valid(current, next_move, '|')
                prev = current
                current = next_move
                next_move = start_pipe_coords
                next_pipe = grid[next_move[1]][next_move[0]]
...
# Fix that last point is not included in index!!!
